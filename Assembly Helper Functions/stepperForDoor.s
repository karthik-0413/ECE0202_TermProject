...
;*******************************************************************************
	...
	AREA    stepper2, CODE, READONLY
	EXPORT	step2				; make __main visible to linker
	ENTRY			
...
step2	PROC
	...
	BIC   r1, r1, #0x00F00000 ; Masking the registers that we are interested in
	BIC   r1, r1, #0x000F0000 ; Masking the registers that we are interested in
	ORR   r1, r1, #0x00500000 ; Masking the registers that we are interested in
	ORR   r1, r1, #0x00050000 ; Masking the registers that we are interested in
	STR   r1, [r0, #GPIO_MODER] ; Storing the value of the MODER to r1
	  ...
	  ; Moving the value needed to move up one floor in our elevator model
      MOV r8, #595
...
delayloop2
	  ...
	  BIC r1, #0x00000F00 ; Masking the registers that we are interested in
	  ORR r1, #0x00000800 ; Setting the desired bits
	  ...
	  LDR r1, [r0, #GPIO_ODR] ; Loading the value of the ODR onto r1
	  BIC r1, #0x00000F00 ; Masking the registers that we are interested in
	  ORR r1, #0x00000400 ; Setting the desired bits
	  STR r1, [r0, #GPIO_ODR] ; Storing the value of the ODR to r1
	  ...
	  LDR r1, [r0, #GPIO_ODR] ; Loading the value of the ODR onto r1
	  BIC r1, #0x00000F00 ; Masking the registers that we are interested in
	  ORR r1, #0x00000200 ; Setting the desired bits
	  STR r1, [r0, #GPIO_ODR] ; Storing the value of the ODR to r1
	  ...
	  LDR r1, [r0, #GPIO_ODR] ; Loading the value of the ODR onto r1
	  BIC r1, #0x00000F00 ; Masking the registers that we are interested in
	  ORR r1, #0x00000100 ; Setting the desired bits
	  STR r1, [r0, #GPIO_ODR] ; Storing the value of the ODR to r1
